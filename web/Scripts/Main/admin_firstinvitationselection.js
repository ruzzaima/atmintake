...
$(function () {
    ...
    viewModel = {
        ...
        firstsuppdoc: ko.mapping.fromJS(firstsuppdoc),
        firstsuppdocori: ko.mapping.fromJS(firstsuppdocori),
        ...
        search: function (d) {
            oTable = $('#applicant_table').dataTable({
                ...
                "bDestroy": true,
                "bServerSide": true,
                "sAjaxSource": server + '/Admin/LoadToUpdateApplicant?category=' + ko.mapping.toJS(viewModel.searchcriteria.category) + "&name=" + ko.mapping.toJS(viewModel.searchcriteria.name) + "&icno=" + ko.mapping.toJS(viewModel.searchcriteria.icno) + "&acquisitionid=" + selectedid,
                ...
                "aoColumns": [
                                { "bSortable": false },
                                { "bSortable": true },
                                { "bSortable": true, "sClass": "text-middle text-center" },
                                { "bSortable": false, "sClass": "text-middle text-center" },
                                {
                                    "bSortable": false,
                                    "sClass": "text-middle text-center",
                                    "mRender": function (data, type, full) {
                                        var id = data;
                                        return '<label><input type="radio" app-data="true" name="firstselection' + id + '" class="check-row-approve" id="cba_' + id + '" value="' + id + '" /> <span style="padding-bottom:5px;">Panggil Pemilihan</span> </label>';
                                    }
                                },
                                {
                                    "bSortable": false,
                                ...
                                },
                                {
                                    ...
                                    "mRender": function (data, type, full) {
                                        var id = data;
                                        return '<label><input type="radio" reject-data="true" name="firstselection' + id + '" class="check-row-reject" id="cbr_' + id + '" value="' + id + '" /> <span style="padding-bottom:5px;">Tidak Terpilih</span> </label>';
                                    }
                                }
                ],
        ...
        uploaddocument: function () {
            ...
            fileupload = $('#filetoupload').fileupload({
                ...
                formData: { uploadtype: 'ACQDOCIS', acquisitionid: selectedid },
                add: function (e, data) {
                    ...
                    if (result.OK) {
                        ...
                        viewModel.firstsuppdocori(docs.DocumentName());
                    } else {
        ...
        downloaddoc: function (d) {
            window.open(server + "/SuppDoc/" + viewModel.firstsuppdoc(), "DownloadDoc", null, true);
        },
        searchselected: function () {
            oTable = $('#selectedapplicant_table').dataTable({
                ...
                "bDestroy": true,
                "bServerSide": true,
                "sAjaxSource": server + '/Admin/LoadSubmittedApplicant?category=' + ko.mapping.toJS(viewModel.searchcriteria.category) + '&acquisitionid=' + selectedid + '&invitationselection=true&statecode=' + ko.mapping.toJS(viewModel.searchcriteria.state) + '&citycode=' + ko.mapping.toJS(viewModel.searchcriteria.city) + '&invitationlocid=' + ko.mapping.toJS(viewModel.searchcriteria.invitationlocid),
                "fnDrawCallback": function (oSettings) {
                    /* Need to redo the counters if filtered or sorted */
                    //if (oSettings.bSorted || oSettings.bFiltered) {
                    for (var i = 0, iLen = oSettings.aiDisplay.length; i < iLen; i++) {
